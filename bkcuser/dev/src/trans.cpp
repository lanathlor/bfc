...
void bkc::trans::setSign(const std::string &str)
{
	this->_sign = str;
}
...
std::string bkc::trans::sign(const trans &t, const bkc::rsaKey &key)
{
	json j;

	j["sender"] = t._sender;
	j["receiver"] = t._receiver;
	j["amount"] = t._amount;
	j["timestamp"] = t._timestamp;

	return (key.signPrintable(j.dump()));
}
